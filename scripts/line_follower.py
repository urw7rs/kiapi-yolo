...
global perr, ptime, serr, dt, move,past_ang
perr = 0
ptime = 0
serr = 0
dt = 0
...
class Follower:
	...
	if M['m00'] > 0:
	  cxm = int(M['m10']/M['m00'])
        ...
        int(width / 2 + 100 * (math.sin((rad_steer_angle)))), int(height / 2 - 100 * ((math.cos(rad_steer_angle)))),),
                 (0, 125, 255), 5, )
        cv2.circle(dst, (
        int(width / 2 + 100 * (math.sin((rad_steer_angle)))), int(height / 2 - 100 * ((math.cos(rad_steer_angle)))),),
                   10, (0, 125, 255), -1, )
	...
	if steer_angle==0:
        ...
        #cx=int(((width / 2 +steer_angle)+cxm)/2)
	cx=int(width / 2 +steer_angle-5)
        err = cx -  (width /2)
        K_p =1.3
